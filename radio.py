...
class rayleigh_chan_lte:
    ...
    def __init__(self, FLAGS, sample_rate=0.96e6, mobile=False, mix=False):
        ...
        self.max_TO = FLAGS.max_TO
        self.min_TO = FLAGS.min_TO
        self.n_rx_samples = int(self.nfft +  self.nfft * 0.25)*self.nSymbol + self.max_TO  # data + CP + range of timing offset
        # print('n_rx_samples: ', self.n_rx_samples)
    ...
    def channel(self, tx_signal, Fd, ch_coeff, alpha_matrix, n_taps, n_sym, n_sc, timing_offset, doppler=False):
        if doppler:
            ...
            gt = np.matmul(a_taps, alpha_matrix)
            # print('gt: ', gt.shape)
            # print('tx_signal: ', tx_signal.shape)
            # rx_signal = np.convolve(gt, tx_signal, mode='same')
            rx_signal = self.convolve_with_delay(tx_signal, gt, timing_offset)
            # print('rx_signal: ', rx_signal.shape)
            Ht = np.fft.fft(gt, self.nfft)
            # print('rx_signal.shape: ', rx_signal.shape)
            # print('rx_signal[0]: ', rx_signal[0])
        return rx_signal, Ht

    def pad_complex_array(self, array):
        current_length = len(array)
        # print('current_length: ', current_length)
        # print('self.n_rx_samples: ', self.n_rx_samples)
        
        if current_length < self.n_rx_samples:
            # Initialize an array of zeros with the desired length and copy the original array into it
            padded_array = np.zeros(self.n_rx_samples, dtype=np.complex64)
            padded_array[:current_length] = array
        else:
            # If the array is already longer than M, truncate it to length M
            padded_array = array[:self.n_rx_samples]
        
        return padded_array

    def convolve_with_delay(self, tx_signal, gt, beta):
        """
        Convolves the transmitted signal with the channel impulse response considering a timing offset beta.
        
        Parameters:
        - tx_signal: Transmitted signal (complex numpy array)
        - gt: Channel impulse response (complex numpy array)
        - beta: Timing offset (integer)
        - num_rx_sample: Number of received signal sample (integer)
        
        Returns:
        - rx_signal: Received signal (complex numpy array) with the length of num_rx_sample
        """
        tx_length = len(tx_signal)
        
        # Pad the transmitted signal at the beginning for positive beta
        if beta > 0:
            tx_signal = np.pad(tx_signal, (beta, 0), 'constant')
        # For negative beta, we need to ensure no negative indexing occurs
        elif beta < 0:
            beta = abs(beta)
            if beta < tx_length:
                tx_signal = tx_signal[beta:]
            else:
                tx_signal = np.array([], dtype=tx_signal.dtype)
        
        # Perform convolution
        full_rx_signal = np.convolve(tx_signal, gt, mode='full')
        
        rx_signal = self.pad_complex_array(full_rx_signal)
        return rx_signal
    ...
    def run(self, inputs):
        ...
        labels = np.zeros((n_fr, self.nfft,), dtype=np.float64)
        if self.chan == 'awgn':
            y_distored = inputs
            ...
            # y_distored = np.zeros(inputs.shape, dtype=np.complex64)
            # y_distored = np.zeros((n_fr,n_samples_fr*2,), dtype=np.float64)
            # y_distored = np.zeros((n_fr,self.n_rx_samples*2,), dtype=np.float64)
            y_distored = np.zeros((n_fr,self.n_rx_samples,2), dtype=np.float64)
            ...
            for i_fr in range(n_fr):
                ...
                tx_signal = np.reshape(inputs[i_fr, :, :], [n_samples_fr, ])
                # if(i_fr == 0):
                #     print('tx_signal shape in for loop: ', tx_signal.shape)
                rx_signal, Ht = self.channel(tx_signal,
                ...
                y_distored[i_fr, :, 0] =  np.real(rx_signal)
                y_distored[i_fr, :, 1] = np.imag(rx_signal)
                labels[i_fr, :, ] = one_hot_vector
                H_fr[i_fr, :, :] = Ht
        ...
        return y_out, labels, H_fr

    def __call__(self, inputs):
        y_out, labels, H_fr = self.run(inputs)
        return y_out, labels, H_fr
...
def AWGN_channel_np(inputs, SNR):
    ...
    sig_pwr = np.square(inputs[:, :, 0:1]) + np.square(inputs[:, :, 1:])
    savg_pwr = np.nanmean(sig_pwr)
    # print('savg_pwr: ', savg_pwr)
    # Normalize the inputs by the average signal power
    inputs_norm = inputs / np.sqrt(savg_pwr)
    # Generate noise with the same shape as inputs
    noise = np.random.randn(shapes[0], shapes[1], shapes[2])
    # Calculate the noise standard deviation
    noisestd = np.sqrt(0.5) * np.power(10.0, (-SNR / 20.0))
    noisestd = np.reshape(noisestd, [-1, 1, 1])
    # Apply the noise standard deviation
    noise = noise * noisestd
    outputs = inputs_norm + noise
